...
class TestBCon(unittest.TestCase):
    ...
    def setUp(self):
        self.con = pdblp.BCon(host=HOST, port=IP_PORT, timeout=5000)
        self.con.start()
    ...
    def pivot_and_assert(self, df, df_exp, with_date=False):
        ...
        if with_date:
            ...
            df_exp.loc[:, 'date'] = pd.to_datetime(df_exp.loc[:, 'date'], format='%Y%m%d')
    ...
    def test_bdh_empty_data_with_non_empty_data(self):
        ...
        df_exp = pd.DataFrame(
            ...
            columns=pd.MultiIndex.from_product(
                [['AAPL US Equity'], ['PX_LAST', 'VOLUME']], names=['ticker', 'field']
            )
        )
        assert_frame_equal(df, df_exp)
    ...
    def test_bdh_one_ticker_one_field_pivoted(self):
        ...
        midx = pd.MultiIndex(levels=[['SPY US Equity'], ['PX_LAST']], labels=[[0], [0]], names=['ticker', 'field'])
    ...
    def test_bdh_one_ticker_one_field_longdata(self):
        df = self.con.bdh('SPY US Equity', 'PX_LAST', '20150629', '20150630', longdata=True)
    ...
    def test_bdh_one_ticker_two_field_pivoted(self):
        ...
        midx = pd.MultiIndex(levels=[['SPY US Equity'], cols], labels=[[0, 0], [0, 1]], names=['ticker', 'field'])
        df_expect = pd.DataFrame(
            index=pd.date_range('2015-06-29', '2015-06-30'),
            columns=midx,
    ...
    def test_bdh_one_ticker_two_field_longdata(self):
        cols = ['PX_LAST', 'VOLUME']
        df = self.con.bdh('SPY US Equity', cols, '20150629', '20150630', longdata=True)
        idx = pd.Index(['date', 'ticker', 'field', 'value'])
        data = [
            ['2015-06-29', '2015-06-29', '2015-06-30', '2015-06-30'],
    ...
    def test_bdh_value_errors(self):
        bad_col = 'not_a_fld'
        self.assertRaises(ValueError, self.con.bdh, 'SPY US Equity', bad_col, '20150630', '20150630')
        bad_ticker = 'not_a_ticker'
        self.assertRaises(ValueError, self.con.bdh, bad_ticker, 'PX_LAST', '20150630', '20150630')

    def test_bdib(self):
        # BBG has limited history for the IntradayBarRequest service so this needs to be periodically updated
        df = self.con.bdib(
            'SPY US Equity', '2018-02-09T10:00:00', '2018-02-09T10:20:01', event_type='BID', interval=10
        )
        idx = pd.DatetimeIndex(['2018-02-09T10:00:00', '2018-02-09T10:10:00', '2018-02-09T10:20:00'])
        data = [
            [260.85, 260.90, 260.50, 260.58, 8038, 938],
    ...
    def test_ref_one_ticker_one_field_override(self):
        df = self.con.ref('AUD Curncy', 'SETTLE_DT', [('REFERENCE_DATE', '20161010')])
        df_expect = pd.DataFrame(
            columns=['ticker', 'field', 'value'],
            data=[['AUD Curncy', 'SETTLE_DT', pd.datetime(2016, 10, 12).date()]]
        )
        assert_frame_equal(df, df_expect)

    def test_ref_invalid_field(self):
        self.assertRaises(ValueError, self.con.ref, 'EI862261 Corp', 'not_a_field')

    def test_ref_not_applicable_field(self):
        # test both cases described in https://github.com/matthewgilbert/pdblp/issues/6
    ...
    def test_ref_applicable_with_not_applicable_field(self):
        ...
        df_exp = pd.DataFrame(
            [['BVIS0587 Index', 'MATURITY', np.NaN], ['BVIS0587 Index', 'NAME', 'CAD Canada Govt BVAL Curve']],
            columns=['ticker', 'field', 'value']
        )
        assert_frame_equal(df, df_exp)
    ...
    def test_bulkref_one_ticker_one_field(self):
        df = self.con.bulkref('BCOM Index', 'INDX_MWEIGHT', ovrds=[('END_DATE_OVERRIDE', '20150530')])
        df_expected = pd.read_csv(os.path.join(self.path, 'bulkref_20150530.csv'))
        self.pivot_and_assert(df, df_expected)

    def test_bulkref_two_ticker_one_field(self):
        df = self.con.bulkref(['BCOM Index', 'OEX Index'], 'INDX_MWEIGHT', ovrds=[('END_DATE_OVERRIDE', '20150530')])
        df_expected = pd.read_csv(os.path.join(self.path, 'bulkref_two_fields_20150530.csv'))
        self.pivot_and_assert(df, df_expected)

    def test_bulkref_singleton_error(self):
        # calling bulkref which returns singleton throws error
        self.assertRaises(ValueError, self.con.bulkref, 'CL1 Comdty', 'FUT_CUR_GEN_TICKER')
    ...
    def test_hist_ref_one_ticker_one_field_numeric(self):
        ...
        df_expect = pd.DataFrame(OrderedDict([
            ('date', dates),
    ...
    def test_bulkref_hist_one_field(self):
        ...
        df = self.con.bulkref_hist('BCOM Index', 'INDX_MWEIGHT', dates=dates, date_field='END_DATE_OVERRIDE')
        df_expected = pd.read_csv(os.path.join(self.path, 'bulkref_20150530_20160530.csv'))
    ...
    def test_bulkhist_ref_with_alternative_reference_field(self):
        ...
        # received
        dates = ['20160625']
        self.con.bulkref_hist('BVIS0587 Index', 'CURVE_TENOR_RATES', dates, date_field='CURVE_DATE')

    def test_context_manager(self):
        with pdblp.bopen(host=HOST, port=IP_PORT) as bb:
    ...
    def test_bsrch(self):
        l = self.con.bsrch('COMDTY:VESSEL')[:5]
        l_expect = ['IMO1000019 Index', 'LADY K II', 'IMO1000021 Index', 'MONTKAJ', 'IMO1000033 Index']
        self.assertEquals(l, l_expect)

    def test_govtlist(self):
        l = self.con.govtlist('T Govt')
